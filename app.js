...
const addTextWatermarkToImage = async function (inputFile, outputFile, text) {
	try {
		...
		console.log(`Text watermark was added succesfully!`);
		await startApp();
	} catch (err) {
		console.log(`Something went wrong / file does not exist`);
	}
};
...
const addImageWatermarkToImage = async function (
	...
	try {
		...
		await image.quality(100).writeAsync(outputFile);
		console.log(`Image Watermark was added succesfully!`);
		await startApp();
	} catch (err) {
		console.log(`Something went wrong / file does not exist`);
	}
...
const startApp = async () => {
	...
	if (options.watermarkType === 'Text watermark') {
		...
		} else console.log(`Err... someething went wrong...`);
	}
};
